...
// in the line content_str.replace(pos - 2, (it->first).length() + 4, it->second)
// the -2 and +4 are taking into account the positions of the brackets to be replaced
// in the line content_str.replace(pos_begin, pos_end - pos_begin + 2, "")
// the +2 takes into account the }} at the end
std::string    dynamic_content(std::string content_str, std::map<std::string, std::string> params)
{
    if (params.empty())
    {
